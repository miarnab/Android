...
public class MainActivity extends Activity {  
   ...
   public void speakText(View view){
	   ...
	   else if(toSpeak.equalsIgnoreCase("Who are you?")){
		   String toSpeak10=" O Hello I am an android application named Hello created by Arnab Mitra, built on eclipse IDE and my support extends from android 4.0 (API 14) to android 4.4 (API 19)";
		   Toast.makeText(getApplicationContext(), toSpeak10,Toast.LENGTH_LONG).show();       
		   ttobj.speak(toSpeak10, TextToSpeech.QUEUE_FLUSH, null);
	   }
	   else if(toSpeak.equalsIgnoreCase("What is android?")){
		   String toSpeak11=" O Hello, Android is an advanced operating system developed upon the Linux kernel , marketed and owned by Google.Android runs on mobiles , tablets , TVs , wearables and cars .Andy Rubin is the father and creator of the Android operating system.";
		   Toast.makeText(getApplicationContext(), toSpeak11,Toast.LENGTH_LONG).show();       
		   ttobj.speak(toSpeak11, TextToSpeech.QUEUE_FLUSH, null);
	   }
	   //Elements of Periodic Table
	   else if(toSpeak.equalsIgnoreCase("Hydrogen")){
		   String toSpeak12=write.getText().toString()+" is a Group-1 Period-1 element having atomic number-1 and mass number-1 and has a valency of +1 and has three isotopes named Protium, Deuterium and Tritium.";
		   Toast.makeText(getApplicationContext(), toSpeak12,Toast.LENGTH_LONG).show();       
		   ttobj.speak(toSpeak12, TextToSpeech.QUEUE_FLUSH, null);
	   }
	   else if(toSpeak.equalsIgnoreCase("Lithium")){
		   String toSpeak13=write.getText().toString()+" is a Group-1 Period-2 element having atomic number-3 and mass number-5 and has a valency of +1";
		   Toast.makeText(getApplicationContext(), toSpeak13,Toast.LENGTH_LONG).show();       
		   ttobj.speak(toSpeak13, TextToSpeech.QUEUE_FLUSH, null);
	   }
	   else if(toSpeak.equalsIgnoreCase("Helium")){
		   String toSpeak14="This information is not available right now, stay tuned for more.";
		   Toast.makeText(getApplicationContext(), toSpeak14,Toast.LENGTH_LONG).show();       
		   ttobj.speak(toSpeak14, TextToSpeech.QUEUE_FLUSH, null);
	   }
	   else if(toSpeak.equalsIgnoreCase("Beryllium")){
		   String toSpeak15=" This information is not available right now, stay tuned for more";
		   Toast.makeText(getApplicationContext(), toSpeak15,Toast.LENGTH_LONG).show();       
		   ttobj.speak(toSpeak15, TextToSpeech.QUEUE_FLUSH, null);
	   }
	   else{
		   String toSpeak16=" O Hello you must be one of the acquaintances of Arnab Mitra.Sorry your name "+write.getText().toString()+" is not listed for any voice outputs / this information is not available in the database right now.";
